...
exports.setSuperadmin = async (req, res) => {
    try {
        ...
        if (user.is_superadmin) {
            return res.status(200).json({ message: 'Người dùng đã có quyền superadmin' });
        }
        // Cấp quyền superadmin cho người dùng bằng cách cập nhật thông tin
       let superadmin = await fn.getDatafindOneAndUpdate(User, { _id: _id },{ is_superadmin:true} );
       if (!superadmin) return utils.errorNotFound(res);
       const edited = await User.findById(superadmin._id);

        return fn.success(res, "Cấp quyền quản trị thành công", { data: edited._doc });
    }
    catch (error) {
        console.log('[ERR]: controllers/qlcnew/user/grantSuperadmin:', error);
        return fn.setError(res, error.message);
